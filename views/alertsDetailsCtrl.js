...
//})
alertsApp.controller("alertsDeatailsCtrl",function ($scope, $location, alerts, activeUser,  $routeParams, Alert){
    if (!activeUser.isLoggedIn()) {
      $location.path("/");
    return;
    }
 $scope.alert = new Alert(alerts.get($routeParams.alertIndex));
}
)
